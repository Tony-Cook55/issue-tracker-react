...
import { FaArrowAltCircleLeft, FaShoppingCart, FaTrash, FaPencilAlt, FaSave } from "react-icons/fa";
// ICONS //
...
export default function BugEditor(){
  ...
  const [bugTitle, setBugTitle] = useState('Bug Title Here');
  const [bugDescription, setBugDescription] = useState('Description Here');
  const [editMode, setEditMode] = useState(false);

  const handleSaveBug = () => {
    // Perform save logic here
    console.log('Bug saved:', { bugTitle, bugDescription });
...
<div className="wrapper        bounce_in_bottom">
    ...
    <div className="bug_title_div">
            <div className="">
              {editMode ? (
                ...
                <>
                  <h1>{bugTitle}</h1>
                  <p>Bugs Id <br /> 123456789023456789</p>
                  <br></br>
                  <h2>{bugDescription}</h2>
                </>
              )}
            </div>
          </div>
  ...
  <div className="bottom_cap_under_accordion">
    ...
    <div className="end_cap_base">
      ...
      {/* eeeeeeeeee EDIT BUTTON eeeeeeeeee */}
      <div className="row">
        <div className="col-md-6 col-sm-4">
          {editMode ? (
            <button className="edit_button" onClick={handleSaveBug}>
              <p className="edit_text"><FaSave /></p>
              <p className="edit_text">Save</p>
            </button>
          ) : (
            <button className="edit_button" onClick={() => setEditMode(true)}>
              <p className="edit_text"><FaPencilAlt /></p>
